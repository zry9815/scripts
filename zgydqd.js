...
//只需要把 zgdyqd 批量替换成你想取的名字
// 把  教程  改成软件名  

const $ = new Env('中国移动签到');
let status;

status = (status = ($.getval("zgdyqdstatus") || "1")) > 1 ? `${status}` : "";

const zgdyqdurlArr = [], zgdyqdhdArr = [], zgdyqdbodyArr = [], zgdyqdcount = ''

let zgdyqdurl = $.getdata('zgdyqdurl')
let zgdyqdhd = $.getdata('zgdyqdhd')
let zgdyqdbody = $.getdata('zgdyqdbody')
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        zgdyqdurlArr.push($.getdata('zgdyqdurl'))
        zgdyqdhdArr.push($.getdata('zgdyqdhd'))
        zgdyqdbodyArr.push($.getdata('zgdyqdbody'))

        let zgdyqdcount = ($.getval('zgdyqdcount') || '1');

        for (let i = 2; i <= zgdyqdcount; i++) {

            zgdyqdurlArr.push($.getdata(`zgdyqdurl${i}`))
            zgdyqdhdArr.push($.getdata(`zgdyqdhd${i}`))
            zgdyqdbodyArr.push($.getdata(`zgdyqdbody${i}`))
        ...
        for (let i = 0; i < zgdyqdhdArr.length; i++) {

            if (zgdyqdhdArr[i]) {

                zgdyqdurl = zgdyqdurlArr[i];
                zgdyqdhd = zgdyqdhdArr[i];
                zgdyqdbody = zgdyqdbodyArr[i];
...
function zgdyqdck() {
    if ($request.url.indexOf("signIn") > -1) {
        ...
        const zgdyqdhd = JSON.stringify($request.headers)
        if (zgdyqdhd) $.setdata(zgdyqdhd, `zgdyqdhd${status}`)
        $.log(zgdyqdhd)

        const zgdyqdbody = $request.body
        if (zgdyqdbody) $.setdata(zgdyqdbody, `zgdyqdbody${status}`)
        $.log(zgdyqdbody)
